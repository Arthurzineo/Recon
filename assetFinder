...
func init() {
	...
	containerName = target + "-" + x + "-assetfinder"
...
func executa() {
	...
	command := fmt.Sprintf(
		"docker run --rm --name %s -v /c/recon/data/%s/temp:/data kali-recon assetfinder -subs-only %s >> C:\\recon\\data\\%s\\temp\\%s",
