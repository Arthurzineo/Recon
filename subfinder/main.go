...
func init() {
	...
	containerName = target + "-" + x + "-subfinder"
...
func executa() {
	hostPath := "C:\\recon\\data\\" + target + "\\temp"
	...
	command := fmt.Sprintf(
		"docker run --rm --name %s -v /c/recon/data/%s/temp:/data kali-recon subfinder -d %s -oJ >> C:\\recon\\data\\%s\\temp\\%s",
		containerName, target, domain, target, saida,
	)
...
func parse() {
	caminhoArquivo := fmt.Sprintf("C:\\recon\\data\\%s\\temp\\%s", target, saida)
	...
	for scanner.Scan() {
		...
		// Decodificar a linha do JSON para a struct Dados
		var dados Dados
		if err := json.Unmarshal([]byte(linha), &dados); err != nil {
			log.Printf("Erro ao decodificar JSON na linha: %s, erro: %v", linha, err)
			continue
		}

		// Adicionar o valor do campo `host` ao mapa, ignorando duplicatas
		if dados.Host != "" { // Verificar se o campo `host` não está vazio
			subdomains[dados.Host] = struct{}{}
		}
	}
